...
function setupEvents() {
    ...
    ['bg-blur', 'bg-darken', 'bg-scale', 'text-animation', 'text-color', 'accent-color', 'shadow-color', 'particle-color', 'font-size', 'meta-artist', 'meta-song', 'wm-opacity', 'trans-font', 'trans-size']
...
function updateParticles(ctx, w, h) {
    if (state.particles.length < 50) state.particles.push({ x: Math.random() * w, y: h + 10, v: 1 + Math.random() * 3, s: 1 + Math.random() * 4 });
